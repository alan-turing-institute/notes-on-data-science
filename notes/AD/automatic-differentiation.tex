...
$y$, etc. Primitives are add, sub, mul, and so on, and come with an
arity; we might use $p$, $q$ etc for a particular, but arbitrary,
primitive. Likewise $E$, $F$, etc are particular, but arbitrary
expressions.
...
The differentiation operator, $D\colon \var\times\expr \to \expr$ is a
transformation operator on expressions. $D_x(E)$ is an expression that
is the derivative of $E$ with respect to~$x$. In this section we give
the rules for computing $D_x(E)$.

We need built-in rules for the derivatives of the built-in primitives
with respect to their arguments. A primitive of arity $n$ has $n$
derivatives, one for each argument. Each derivative is a function,
also of arity $n$, but not necessarily a primitive. This is a problem,
because we don't have functions in the language. For now, we solve
this problem by writing these derivatives as functions in the host
language: they take, as arguments, expressions, and substitute those
expressions into the appropriate placeholders.  

For $p$ a primitive of arity $n$, and for any $i\in\{1, \dotsc, n\}$ we
denote by $[D_ip]$ the function of arity $n$, whose arguments are
expressions, such that $[D_ip](E_1, E_2, \dotsc)$ is an $\expr$ which
computes the derivative of $p$ with respect to its $i$th argument. 

Also, to write down the chain rule 

The rules are as follows. Here, $n$ is a~$\num$.
\begin{equation*}
  \begin{aligned}
