...
\sidenote*{\emph{Exercise:} The figure below shows a
...
vector.\sidenote{There is another sense of ``vector,'' which I suspect
  causes confusion. That is the computer scientists notion of ``a
  list-like data structure'' (often with $O(1)$ access time). For that
  notion, we shall the terms ``pair,'' (for two) ``triple,'' (for
  three) and ``tuple'' (in general).}
...
\sidenote{There \emph{is} one important application}
