...
sets. Suppose $X$ and $Y$ are sets. Their \defn{Cartesian product} (or
...
maps relevant to that topic.\sidenote{The term “function” is sometimes
...
\[\begin{tikzcd}
  ...
  \end{tikzcd}\]
  \caption{The identity map $\id_X\colon X\to X$.}
\end{sidefigure}
...
applying $f$,” and we normally read $g\circ f$ as “$g$ after $f$.”

\eg\ For any map $f\colon A\to B$, $f\circ\id_A = \id_B\circ f = f$.
...
and $g$ differed on any $x\in X$, then $i$ would have to take $f(x)$ and
$g(x)$ to different points in $B$, otherwise it would not be
injective, and so the compositions could not be equal.) That is,
injective maps are “maps that can be cancelled out on the left.”
...
injective maps are “maps that can be cancelled out on the right.”
...
injective and surjective, shown in figures~\ref{fig:injection}
...
edges. There are certain sets for which multiple paths exist between
the two. In figure~\ref{fig:injection}, for example, one path from $X$
to $B$ is “first $f$ then $i$” and another path is “first $g$ then
$i$.” The premise in the definition of injective is precisely that “it
...
inverse is called an \defn{bijection} (or an \defn{isomorphism of
  sets}).\sidenote{There seem to be two names for everything:
