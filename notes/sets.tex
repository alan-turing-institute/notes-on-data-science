...
There is a convenient, graphical, way to give the in the previous two
examples. Consider each diagram of figure~\ref{fig:injection}
and~\ref{fig:surjection} as directed graphs, with the sets forming
the vertices and the maps forming the edges. There may be certain
pairs of sets for which multiple paths exist between the two. For
example, in figure~\ref{fig:injection} one path from $X$ to $B$ is
“first $f$ then $i$” and another path is “first $g$ then $i$.”  The
premise in the definition of injective is precisely that “it doesn't
