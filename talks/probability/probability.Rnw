...
\usepackage[paperwidth=209.6mm, paperheight=157.2mm,
  left=0.1875in, top=0.1875in, bottom=0.1875in, footskip=0.164in, textwidth=100mm,
...
Times. From \textcite{jones2004}. The sample space is $\mathcal{A} = \{\text{a},
\text{b}, \dotsc, \text{z}\}$ and the probability mass function is $p(\text{a})
= \Sexpr{nyt$frequency[nyt$letter == "a"]}$, etc. 
\begin{center}
...
%%
\label{slide:entropy}%
The Shannon information content of the `\Sexpr{l}' is
\[
\log_2 \frac{1}{P(\text{\Sexpr{l}})} = \Sexpr{log2(1/nyt$frequency[nyt$letter ==
    l])}\,\text{bits}.
\]
The entropy of the the ensemble of letter frequencies is
\[
\sum_{l\in\mathcal{A}} P(l) \log_2\frac{1}{P(l)}
= \Sexpr{with(nyt, sum(frequency * log2(1/frequency)))} \,\text{bits}.
\]

Here's a sample of 100 characters from this ensemble:
<<monogram_sample, size="small", comment=''>>=
monogram_sample <- sample(1:26, size = 100, prob = nyt$frequency, replace = TRUE)
cat(letters[monogram_sample], sep = "", fill = 50)
@ 
Out of all sequences of length 100:
\[
P_m(\text{\Sexpr{paste0(letters[monogram_sample[1:5]],
    collapse='')}...\Sexpr{paste0(letters[monogram_sample[99:100]],
    collapse='')}})
= P(\text{\Sexpr{letters[monogram_sample[1]]}})
  P(\text{\Sexpr{letters[monogram_sample[2]]}})
  P(\text{\Sexpr{letters[monogram_sample[3]]}}) \dotsm
  P(\text{\Sexpr{letters[monogram_sample[100]]}})
= \Sexpr{prod(nyt$frequency[monogram_sample])}.
\]
Really small probabilities are difficult to interpret. The information
content of this sequence is
$\Sexpr{sum(log2(1/nyt$frequency[monogram_sample]))}$ bits (or
$\Sexpr{sum(log2(1/nyt$frequency[monogram_sample]))/length(monogram_sample)}$
bits per letter).

Here is a set of letters sampled \emph{uniformly} from the alphabet:
<<uniform_letter_sample, size="small", comment=''>>=
letter_sample <- sample.int(26, 100, replace = TRUE)
cat(letters[letter_sample], sep="", fill = 50)
@ 
It has an information content of
$\Sexpr{sum(log2(1/nyt$frequency[letter_sample]))/length(letter_sample)}$ bits
per letter (with respect to the `true' ensemble). 
...
<<chiphertext, size="small", comment=''>>=
cat(as.character(ciphertext), fill = 50, sep = "")
@

What is $\pi$ and what is the plaintext?
\newpage
...
We write $P(s \mid f)$ for conditional probability that the second letter in a pair
is $s$, given that the first letter is $f$.

Example: $P(\text{u}\mid \text{q}) = \Sexpr{with(nyt2, frequency[second == "u" &
    first == "q"]/sum(frequency[first == "q"]))}$ whereas $P(\text{qu}) =
...
with the correct bigram distribution,
\[
...
cat(letters[bigram_sample], sep = "", fill = 50)
@

\[
\begin{aligned}
h_b(\text{\Sexpr{paste0(bsl(1:4), collapse = '')}...\Sexpr{paste0(bsl(99:100),
    collapse = '')}})
= \log_2\Bigl(\frac{1}{P_b(\text{\Sexpr{paste0(bsl(1:4), collapse = '')}...\Sexpr{paste0(bsl(99:100),
    collapse = '')}})}\Bigr) 
&= \Sexpr{-bigram_log_likelihood(bigram_sample, bigram)}\,\text{bits}
\end{aligned}
\]
...
\begin{tabular}{@{}llcr@{}}
  \toprule
  Distribution & & Example & Entropy (bits) \\
  \midrule
...
\begin{center}
  ...
  Expected                  & $\Sexpr{with(nyt2, sum(frequency * log2(1/frequency))) - with(nyt, sum(frequency
  * log2(1/frequency)))}$\\                   
  \bottomrule
\end{tabular}
\end{center}
...
And the decryption with the best key found is:
<<ml_plaintext2, size="small", comment=''>>=
plaintext2 <- decipher(perm, ciphertext)
cat(letters[plaintext2], fill = 50, sep = "")
@
...
%% ============================================================================
%%
\newgeometry{left=0.5in, top=0.25in, bottom=0.25in, textwidth=30em}
\newpage
\section*{Notes}
...
write $P(x)$ or $P(\text{a})$. (This is confusing.)
...
The \emph{Shannon entropy}, $H$, of an ensemble, $X$, is
\[
...
often people don't bother to distinguish the $P$s.
...
\]
One way to sample from a distribution with the correct bigram probabilities is
to assume that each letter is independent of all previous letters except
the preceding one. So, for three letters, we would choose a sequence $l_1 l_2 l_3$ with probability:
\[
P(l_1 l_2 l_3) = P(l_1) P(l_2\mid l_1) P(l_3\mid l_2).
\]
...
You might wonder what is so interesting about this.
